...
%   less.
    ...
    for i = 1:1:length(regs)
      ...
      reg = regs(i);
      [params, num_layers] = FullyConnectedNet_Init([H1, H2], D, C, 0, 0, reg, 5e-3)

      [loss, grads, scores] = FullyConnectedNet_loss(X, y, params, num_layers, reg); 
      disp('Initial loss: ')
      loss

      grad_num = eval_numerical_gradient_params(X,y,h,params,reg, num_layers,'FullyConnectedNet_loss')
      ...
      dW1_num = cell2mat(grad_num(1));
      dW1_num_dim = size(dW1_num);
      db1_num = cell2mat(grad_num(2));
      db1_num_dim = size(db1_num);
      dW2_num = cell2mat(grad_num(3));
      dW2_num_dim = size(dW2_num); 
      db2_num = cell2mat(grad_num(4));
      db2_num_dim = size(db2_num); 
      dW3_num = cell2mat(grad_num(5));
      dW3_num_dim = size(dW3_num); 
      db3_num = cell2mat(grad_num(6));
      ...
      db3_error = rel_error(db3_num, db3);
    end
