...
for i = 1:1:iterations_required
    i
    index  = 1;
    W4_subset = W4_single(:,1+channel_counter:channels_allowed+channel_counter, :,:); 
...
%                 OW; 
%                 mult = W4_subset(Filters,:,:,:).*image_subset_padded(:,:,OH:OH+WH-1, OW:OW+WW-1); 
%                 size(mult);
...
%     fileID = fopen('.\conv4_compare.txt','a');
...
%     fclose(fileID); 
%     
%     fileID = fopen('.\conv4_compare_8bit.txt','a');
...
%     fclose(fileID);
%     
    fileID = fopen('.\script_bins\conv4_compare.bin','a');
    for j = 1:1:length(conv_subset)
        data = conv_subset(j,:); 
...
SH = 0;
SW = 0; 
W4_1 = W4_single(F,1+8*(iterations_required-1),:,:); 
W4_1 = permute(W4_1,[3,4,1,2]); 

W4_2 = W4_single(F,2+8*(iterations_required-1),:,:); 
W4_2 = permute(W4_2,[3,4,1,2]); 

W4_3 = W4_single(F,3+8*(iterations_required-1),:,:); 
W4_3 = permute(W4_3,[3,4,1,2]); 

W4_4 = W4_single(F,4+8*(iterations_required-1),:,:); 
W4_4 = permute(W4_4,[3,4,1,2]); 

W4_5 = W4_single(F,5+8*(iterations_required-1),:,:); 
W4_5 = permute(W4_5,[3,4,1,2]); 

W4_6 = W4_single(F,6+8*(iterations_required-1),:,:); 
W4_6 = permute(W4_6,[3,4,1,2]); 

W4_7 = W4_single(F,7+8*(iterations_required-1),:,:); 
W4_7 = permute(W4_7,[3,4,1,2]); 

W4_8 = W4_single(F,8+8*(iterations_required-1),:,:); 
W4_8 = permute(W4_8,[3,4,1,2]); 
...
mult_1 = image_1(1:3,1:3).*W4_1; 
mult_2 = image_2(1:3,1:3).*W4_2; 
mult_3 = image_3(1:3,1:3).*W4_3; 
mult_4 = image_4(1:3,1:3).*W4_4; 
mult_5 = image_5(1:3,1:3).*W4_5; 
mult_6 = image_6(1:3,1:3).*W4_6; 
mult_7 = image_7(1:3,1:3).*W4_7; 
mult_8 = image_8(1:3,1:3).*W4_8; 
