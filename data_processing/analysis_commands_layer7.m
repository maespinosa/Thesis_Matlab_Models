...
[WF,WC, WH, WW] = size(W7_single); 
%image = permute(image_4d,[1,4,2,3]); 
...
for z = 1:1:channel_iterations_required
    ...
    %W7_subset = W7_single(1+filters_in_subset*(i-1):filters_in_subset+filters_in_subset*(i-1),1+channel_counter:channels_allowed+channel_counter, :,:); 
    W7_subset = W7_single(1:filters_in_subset,1+channel_counter:channels_allowed+channel_counter, :,:); 
    %size(W7_subset)
    W7_subset = reshape(permute(W7_subset, [4,3,2,1]),[WW*WH*channels_allowed,filters_in_subset]); 
    %size(W7_subset)
    ...
    mult = transpose(W7_subset)*image_subset_reshaped;
    if(z==1) 
        mult1 = mult + b7_read; 
        size(mult1);
    else 
...
%     fclose(fileID); 
%     
    fileID = fopen('.\script_bins\fc7_compare.bin','a');
    for j = 1:1:length(affine_subset)
...
fileID = fopen('.\script_bins\fc7_compare_summed.bin','w');
for j = 1:1:length(affine_subset)
    data = affine_subset(j,:); 
    fwrite(fileID,data,'single');
end 
...
F = 1; 
CI = 1; 
W7_1 = W7_single(F,1+channels_allowed*(CI-1),:,:); 
W7_1 = permute(W7_1,[3,4,1,2]); 

W7_2 = W7_single(F,2+channels_allowed*(CI-1),:,:); 
W7_2 = permute(W7_2,[3,4,1,2]); 

W7_3 = W7_single(F,3+channels_allowed*(CI-1),:,:); 
W7_3 = permute(W7_3,[3,4,1,2]); 

W7_4 = W7_single(F,4+channels_allowed*(CI-1),:,:); 
W7_4 = permute(W7_4,[3,4,1,2]); 
...
mult_1 = image_1(1:IH,1:IW).*W7_1; 
mult_2 = image_2(1:IH,1:IW).*W7_2; 
mult_3 = image_3(1:IH,1:IW).*W7_3; 
mult_4 = image_4(1:IH,1:IW).*W7_4; 
