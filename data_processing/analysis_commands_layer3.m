...
for i = 1:1:iterations_required
    i
    index  = 1;
    W3_subset = W3_single(:,1+channel_counter:channels_allowed+channel_counter, :,:); 
...
%                 OW; 
%                 mult = W3_subset(Filters,:,:,:).*image_subset_padded(:,:,OH:OH+WH-1, OW:OW+WW-1); 
%                 size(mult);
...
%     fileID = fopen('.\conv3_compare.txt','a');
...
%     fclose(fileID); 
%     
%     fileID = fopen('.\conv3_compare_8bit.txt','a');
...
%     fclose(fileID);
%     
    fileID = fopen('.\script_bins\conv3_compare.bin','a');
    for j = 1:1:length(conv_subset)
        data = conv_subset(j,:); 
...
SH = 0;
SW = 0; 
W3_1 = W3_single(F,1+8*(iterations_required-1),:,:); 
W3_1 = permute(W3_1,[3,4,1,2]); 

W3_2 = W3_single(F,2+8*(iterations_required-1),:,:); 
W3_2 = permute(W3_2,[3,4,1,2]); 

W3_3 = W3_single(F,3+8*(iterations_required-1),:,:); 
W3_3 = permute(W3_3,[3,4,1,2]); 

W3_4 = W3_single(F,4+8*(iterations_required-1),:,:); 
W3_4 = permute(W3_4,[3,4,1,2]); 

W3_5 = W3_single(F,5+8*(iterations_required-1),:,:); 
W3_5 = permute(W3_5,[3,4,1,2]); 

W3_6 = W3_single(F,6+8*(iterations_required-1),:,:); 
W3_6 = permute(W3_6,[3,4,1,2]); 

W3_7 = W3_single(F,7+8*(iterations_required-1),:,:); 
W3_7 = permute(W3_7,[3,4,1,2]); 

W3_8 = W3_single(F,8+8*(iterations_required-1),:,:); 
W3_8 = permute(W3_8,[3,4,1,2]); 
...
mult_1 = image_1(1:3,1:3).*W3_1; 
mult_2 = image_2(1:3,1:3).*W3_2; 
mult_3 = image_3(1:3,1:3).*W3_3; 
mult_4 = image_4(1:3,1:3).*W3_4; 
mult_5 = image_5(1:3,1:3).*W3_5; 
mult_6 = image_6(1:3,1:3).*W3_6; 
mult_7 = image_7(1:3,1:3).*W3_7; 
mult_8 = image_8(1:3,1:3).*W3_8; 
