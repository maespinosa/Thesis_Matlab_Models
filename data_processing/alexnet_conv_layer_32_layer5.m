...
W5_read_single = fread(W5_file,[num_filters*num_channels*height*width,1],'double');
W5_single = zeros(height,width,num_channels,num_filters);

% Hex_W5_read_single = []; 
% for i=1:1:length(W5_read_single)
%     Hex_W5_read_single = [Hex_W5_read_single; dec_to_hex_single(W5_read_single(i))];
% end
...
% 
% for i = 1:1:length(W5_read_single)
%         hex = dec_to_hex_single(W5_read_single(i));  %
...
% 
% for i = 1:1:length(W5_read_single)
%     weight_data = [weight_data; hex_to_dec_single(Hex_W5_read_single(i,:))]; 
% end 
    ...
    for channels = 1:1:num_channels
        for HH = 1:1:width%filters = 1:1:num_filters %channels = 1:1:num_channels
            for WW = 1:1:width%channels = 1:1:num_channels
                W5_single(HH,WW,channels,filters) = W5_read_single(index,1); 
                index = index + 1; 
            end
        end 
    end 
...
W5_single = permute(W5_single,[4,3,1,2]); 
...
b5_read = fread(b5_file,[num_filters,1],'double');
b5_single = zeros(num_filters);

Hex_b5_read = []; 
for i=1:1:length(b5_read)
    Hex_b5_read = [Hex_b5_read; dec_to_hex_single(b5_read(i))];
end
...
% 
% for i = 1:1:length(b5_read)
%         hex = dec_to_hex_single(b5_read(i));  %
...
%     bias_data = [bias_data; hex_to_dec_single(Hex_b5_read(i,:))];
% end 
...
%% Get Input Data
disp('Get Input Data')
MP1_file = fopen('.\python_bins\relu4.bin');
% Essential Parameters 
H = 13; 
W = 13; 
C = 384;  
...
for i = 1:1:length(MP1_read_single)
    ...
    maxpool_data = [maxpool_data; hex_to_dec_single(Hex_MP1_read_single(i,:))]; 
end 
%% Rearrange Data into Image Matrix
disp('Rearrange Data into Image Matrix')
index = 1; 
...
[out, cache] = conv_forward_naive(image_4d, W5_single, bias_data, conv_param); 
...
fileID = fopen('.\script_txts\conv5_truth_data_32.txt','w');
for i = 1:1:length(out_data_reshape)
    data = hex_out_data(i,:);  
    fprintf(fileID, '%s\n',data);
end 
...
fileID = fopen('.\script_txts\conv5_truth_data_8bit.txt','w');
...
fileID = fopen('.\script_bins\conv5_truth_data_8bit.bin','w');
...
[max_out, cache] = max_pool_forward_naive(relu5, pool_param);
...
fileID = fopen('.\script_txts\mp3_truth_data_32.txt','w');
for i = 1:1:length(max_out_data_reshape)
    data = hex_out_data(i,:); 
    fprintf(fileID, '%s\n',data);
end
...
fileID = fopen('.\script_txts\mp3_truth_data_8bit.txt','w');
...
fileID = fopen('.\script_bins\mp3_truth_data.bin','w');
for i = 1:1:length(max_out_data_reshape)
...
% 
% 
% sim_file = fopen('.\conv5_output.txt','r'); 
