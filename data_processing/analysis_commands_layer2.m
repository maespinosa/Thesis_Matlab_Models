...
[WF,WC, WH, WW] = size(W2_single); 
[N, IC, IH, IW] = size(image_4d); 

channels_allowed = 6; %floor(33/5); 
channel_counter = 0; 
...
for i = 1:1:iterations_required
    i
    index  = 1;
    W2_subset = W2_single(:,1+channel_counter:channels_allowed+channel_counter, :,:); 
...
%                 OW; 
%                 mult = W2_subset(Filters,:,:,:).*image_subset_padded(:,:,OH:OH+WH-1, OW:OW+WW-1); 
%                 size(mult);
...
%     end; 
    ...
    fileID = fopen('.\script_txts\conv2_compare.txt','a');
    ...
    fileID = fopen('.\script_txts\conv2_compare_8bit.txt','a');
    ...
    fileID = fopen('.\script_bins\conv2_compare.bin','a');
    for j = 1:1:length(conv_subset)
        data = conv_subset(j,:); 
...
SH = 0;
SW = 0; 
W2_1 = W2_single(F,1+6*(iterations_required-1),:,:); 
W2_1 = permute(W2_1,[3,4,1,2]); 

W2_2 = W2_single(F,2+6*(iterations_required-1),:,:); 
W2_2 = permute(W2_2,[3,4,1,2]); 

W2_3 = W2_single(F,3+6*(iterations_required-1),:,:); 
W2_3 = permute(W2_3,[3,4,1,2]); 

W2_4 = W2_single(F,4+6*(iterations_required-1),:,:); 
W2_4 = permute(W2_4,[3,4,1,2]); 

W2_5 = W2_single(F,5+6*(iterations_required-1),:,:); 
W2_5 = permute(W2_5,[3,4,1,2]); 

W2_6 = W2_single(F,6+6*(iterations_required-1),:,:); 
W2_6 = permute(W2_6,[3,4,1,2]); 
...
mult_1 = image_1(1:5,1:5).*W2_1; 
mult_2 = image_2(1:5,1:5).*W2_2; 
mult_3 = image_3(1:5,1:5).*W2_3; 
mult_4 = image_4(1:5,1:5).*W2_4; 
mult_5 = image_5(1:5,1:5).*W2_5; 
mult_6 = image_6(1:5,1:5).*W2_6; 

adder1 = sum(mult_1,1) + sum(mult_2,1) + sum(mult_3,1) + sum(mult_4,1) + sum(mult_5,1) + sum(mult_6,1); 
